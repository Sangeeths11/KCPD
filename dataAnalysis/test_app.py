...
@st.cache_data
def load_offenses():
    offenses = pd.read_csv("../data/mergedData/merged_df_03.csv", usecols=["Offense_Description"], low_memory=False)
    offenses = offenses["Offense_Description"].unique()
    return offenses

@st.cache_data
def load_crime_data(start_date=None, end_date=None, offense=None):
    crime_df = pd.read_csv("../data/mergedData/merged_df_03.csv", usecols=["Location", "Offense_Description", "Address", "Reported_Date", "dist_id"], low_memory=False)
...
if len(crime_df) > 1000:
    st.warning("Data too big please choose other filters!")
else:
    ...
    if st.sidebar.checkbox("MarkerCluster"):
        m = MarkerCluster().add_to(m)
